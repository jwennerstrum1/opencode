...
Multiplex concurrent agent sessions on a running server — `spawn`, `list`, `switch`, and `close`.

```bash
opencode session mux [command]
```

See [Multiplexing](/docs/multiplexing) for the full workflow, including the interactive session switcher and its keybinds.
